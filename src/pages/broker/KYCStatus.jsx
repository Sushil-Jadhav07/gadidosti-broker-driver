          ...
          <div className="p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {FIELDS.map(({ key, label, icon: Icon }) => (
                <div key={key} className="bg-slate-50 rounded-xl p-3.5 min-w-0">
                  <p className="flex items-center gap-1.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1.5">
                    {Icon && <Icon size={11} className="flex-shrink-0" />} {label}
                  </p>
                  <p className="text-sm font-mono font-semibold text-slate-800 truncate" title={submission?.documents?.[key]}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              {Object.entries(PHOTO_FIELDS).map(([key, { label }]) => (
                <div key={key} className="bg-slate-50 rounded-xl p-3.5 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">{label} Photo</p>
                    {docUrls[key] ? (
                      <p className="text-xs text-emerald-600 font-semibold flex items-center gap-1"><CheckCircle2 size={12} /> Uploaded</p>
                    ) : (
                  ...
                  {docUrls[key] && (
                    <a
                      href={docUrls[key]} target="_blank" rel="noreferrer"
                      className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-xs font-semibold hover:bg-primary/20 transition-colors"
                    >
                      <Eye size={13} /> View
                    </a>
                  )}
