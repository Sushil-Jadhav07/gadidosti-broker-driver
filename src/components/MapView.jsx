...
// Shared map for every screen in this app that needs one (driver's route view, broker's
// fleet view) — accepts routes (drawn via the Directions API) and markers (plain pins) as
// props so neither side reimplements map plumbing. Loading/error states reuse this app's
// existing spinner styling (see NotificationBell/ChatWindow/Login for the same pattern).
export default function MapView({ routes = [], markers = [], height = "400px", className = "", zoom }) {
  ...
  if (loadError) {
    return (
      <div className={`flex flex-col items-center justify-center bg-slate-50 rounded-xl ${className}`} style={{ height }}>
        <p className="text-sm text-slate-400">Couldn't load the map right now.</p>
      </div>
    );
  }
