      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100">
            {["Booking", "Date", "Amount", "Platform Fee", "Net"].map((label) => (
              ...
              <Fragment key={group.label}>
                ...
                {group.rows.map((row) => (
                  <tr key={row.id} className="table-row">
                    <td className="px-4 py-3 font-mono text-xs text-slate-600">{bookingRef({ bookingNumber: row.bookingNumber, bookingId: row.booking_id || row.bookingId })}</td>
                    <td className="px-4 py-3 text-slate-600">{formatDate(row.date || row.created_at || row.createdAt)}</td>
                    <td className="px-4 py-3 text-slate-800 font-semibold">{formatCurrency(row.amount)}</td>
                    <td className="px-4 py-3 text-slate-600">{formatCurrency(row.platformFee)}</td>
                    <td className="px-4 py-3 text-emerald-700 font-semibold">{formatCurrency(row.netEarnings)}</td>
                  </tr>
                ))}
              </Fragment>
            );
          })}
          {!rows.length && <tr><td colSpan={5} className="px-4 py-10 text-center text-slate-400">No earnings found.</td></tr>}
        </tbody>
      </table>
    </div>
  );
