...
const STATUS_LABEL = {
  ...
  offline: { text: "Offline", className: "bg-slate-100 text-slate-500" },
};

// Custom div-based dropdown (no native <select>) — lists every driver, showing their
...
export default function DriverDropdown({ drivers = [], value, onChange, placeholder = "Select a driver" }) {
  ...
  const selected = drivers.find((d) => (d.id || d.user_id) === value);
        ...
        >
          {!drivers.length && (
            <div className="px-3 py-3 text-xs text-slate-400 flex items-center gap-2">
              <User size={14} className="opacity-40" /> No drivers found
            </div>
          )}
          {drivers.map((driver) => {
            const id = driver.id || driver.user_id;
            const isAvailable = driver.status === "available";
            const status = STATUS_LABEL[driver.status] || { text: driver.status, className: "bg-slate-100 text-slate-500" };
            return (
              <div
                key={id}
                role="option"
                ...
                onClick={() => { if (isAvailable) { onChange(id); setOpen(false); } }}
              ...
              >
                ...
                <span className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  {value === id && <Check className="w-3.5 h-3.5 text-primary flex-shrink-0" strokeWidth={3} />}
                </span>
              </div>
            );
          })}
