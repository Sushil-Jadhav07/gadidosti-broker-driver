                  ...
                  <div key={item.path} className="relative"
                    ...
                    <NavLink to={item.path}
                      className={({ isActive: a }) =>
                        `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all relative ${a ? "text-primary" : "hover:bg-white/[0.04]"}`
                      }
                      style={({ isActive: a }) =>
                        a ? { background: "rgba(25,118,255,0.1)", borderLeft: "2px solid #1976FF" } : {}
                      }
                    >
                      <Icon className="flex-shrink-0 transition-colors" size={18}
      ...
      <div className="flex-shrink-0 px-2 pb-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className={`flex items-center gap-3 px-3 py-3 ${(!isExpanded) ? "lg:flex-col lg:items-center lg:gap-1" : ""}`}>
          ...
          <div className={`flex-1 min-w-0 ${(!isExpanded) ? "lg:hidden" : ""}`}>
            <div className="text-white text-[13px] font-medium truncate">{user?.name || "Suresh Patel"}</div>
            <div className="text-[11px]" style={{ color: "rgba(255,255,255,0.35)" }}>Broker</div>
          </div>
        ...
        <button onClick={toggleExpanded}
          className="hidden lg:flex w-full items-center justify-center py-2 rounded-lg transition-all hover:bg-white/[0.04] mt-1">
          {isExpanded
